...
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| `ops-smoke-checks` `verify_pg_trgm` 경로 활성화 | Codex | P2 | `blocked` | workflow_dispatch에서 `verify_pg_trgm=true` 실행이 PASS(`OPS_HEALTH_REQUIRE_PG_TRGM=1` 기준) | 운영 DB `pg_trgm` 확장 설치(현재 `search.pgTrgm.enabled=false`로 FAIL) |
